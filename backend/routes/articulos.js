...
router.post("/api/articulos/", async function (req, res) {
    ...
    in: 'body',description: 'nuevo Artículo',
    ...
    try {
        ...
        res.status(200).json(data.dataValues); // devolvemos el registro agregado recien cargado
    } catch (error) {
...
router.put("/api/articulos/:id", async function (req, res) {
    ...
    try {
        ...
        if (!item) {
            res.status(404).json({ message: "Articulo no encontrado en el sistema" });
            return;
        }
